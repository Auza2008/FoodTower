...
import cn.foodtower.module.modules.render.Wings;
...
public class RenderWings extends  ModelBase {
   ...
   public void renderWings( float partialTicks) {
      ...
      double rotate = this.interpolate(per?mc.thePlayer.rotationYaw: (( ModuleManager.getModuleByClass( KillAura.class).isEnabled() && KillAura.curTarget != null)? Client.Yaw: mc.thePlayer.prevRenderYawOffset), per?mc.thePlayer.rotationYaw:((ModuleManager.getModuleByClass(KillAura.class).isEnabled() && KillAura.curTarget != null ||Client.instance.getModuleManager().getModuleByClass( Scaffold.class).isEnabled())?Client.Yaw: mc.thePlayer.renderYawOffset), partialTicks);
      GL11.glPushMatrix();
