...
public class NotificationRenderer implements INotificationRenderer {
    private final ResourceLocation logo = new ResourceLocation("FoodTower/music.png");

    public void draw(List<Notification> notifications) {
        Minecraft mc = Minecraft.getMinecraft();
        ScaledResolution scaledRes = new ScaledResolution(mc);
        float y = (float) ScaledResolution.getScaledHeight() - (float) (notifications.size() * 32 + 12 + (mc.currentScreen instanceof GuiChat ? 12 : 0));
        ...
        for (Iterator<?> var5 = notifications.iterator(); var5.hasNext(); y += 32.0F) {
            ...
            float x = (float) (ScaledResolution.getScaledWidth() - 5 - 1 - 63) - (Math.max(headerWidth, subHeaderWidth));
            GL11.glPushMatrix();
            GL11.glEnable(3089);
            RenderUtil.prepareScissorBox((int) not.translate.getX(), not.translate.getY(), ScaledResolution.getScaledWidth(), not.translate.getY() + 30F);
            ...
            RenderUtil.drawFastRoundedRect(x - (not.getType().equals(Notifications.Type.MUSIC) ? 2 : 0), not.translate.getY(), ScaledResolution.getScaledWidth() - 5, not.translate.getY() + 30F, 2, new Color(10, 10, 10, 200).getRGB());
            ...
            RenderUtil.triangle(ScaledResolution.getScaledWidth() - 5, not.translate.getY() + 5 + 15, ScaledResolution.getScaledWidth() - 5, not.translate.getY() + 5 + 5, ScaledResolution.getScaledWidth(), not.translate.getY() + 5 + 10, new Color(10, 10, 10, 200).getRGB());
            ...
            if (not.checkTime() >= not.getDisplayTime() + not.getStart()) {
                ...
                not.setTarX(ScaledResolution.getScaledWidth());
                if (not.translate.getX() >= (float) ScaledResolution.getScaledWidth()) {
                    if (not.getType().equals(Notifications.Type.MUSIC))
