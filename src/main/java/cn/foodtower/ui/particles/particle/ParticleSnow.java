package cn.foodtower.ui.particles.particle;
...
//      }
    ...
    private void resetPos() {
        if (this.vector.x < -10.0F) {
            this.vector.x = (float) ScaledResolution.getScaledWidth() + 10;
        }

        if (this.vector.y < -10.0F) {
            sizes = this.random.nextInt(4) + 3;
            this.vector.y = (float) ScaledResolution.getScaledHeight() + 10;
        }
