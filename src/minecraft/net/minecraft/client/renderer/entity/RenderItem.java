...
public class RenderItem implements IResourceManagerReloadListener {
        ...
        if (Client.instance.getModuleManager().getModuleByClass(EnchantEffect.class).isEnabled()) {
            if (!Config.isCustomItems() || CustomItems.isUseGlint()) {
                if (!Config.isShaders() || !Shaders.isShadowPass) {
                    ...
                    this.renderModel(model, ColorUtils.rainbow(0L, 1.0f).getRGB());
                    GlStateManager.popMatrix();
                    GlStateManager.pushMatrix();
                    GlStateManager.scale(8.0F, 8.0F, 8.0F);
    ...
    private void draw(WorldRenderer renderer, float x, float y, float width, float height, float red, float green, int blue, int alpha) {
