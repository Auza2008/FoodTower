...
public class ClickGUI extends GuiScreen implements GuiYesNoCallback {
    ...
    public static Module currentModule = Client.instance.getModuleManager().getModulesInType(currentModuleType).size() != 0
            ? Client.instance.getModuleManager().getModulesInType(currentModuleType).get(0)
            : null;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawImage(new ResourceLocation("foam/hudedit.png"), 4, height - 36, 32, 32);
        ...
        Gui.drawRect(startX + 200, startY, startX + 420, startY + 320,
                new Color(40, 40, 40, (int) opacity.getOpacity()).getRGB());
        FontManager.F18.drawString("    " + "foam" + " " + Client.instance.version, startX + 4, startY + 4, -1);
        for (int i = 0; i < ModuleType.values().length; i++) {
            ...
            if (iterator[i] != currentModuleType) {
                ...
                RenderUtil.drawFilledCircle(startX + 30, startY + 30 + i * 40, 15,
                        new Color(0, 166, 255, (int) opacity.getOpacity()).getRGB(), 5);
            }
            RenderUtil.drawImage(new ResourceLocation("foam/clickicon/" + iterator[i].name().toLowerCase() + ".png"), (int) (startX + 20), (int) (startY + 20 + i * 40), 20, 20);
            try {
                if (this.isCategoryHovered(startX + 15, startY + 15 + i * 40, startX + 45, startY + 45 + i * 40, mouseX,
                    ...
                    currentModule = Client.instance.getModuleManager().getModulesInType(currentModuleType).size() != 0
                            ? Client.instance.getModuleManager().getModulesInType(currentModuleType).get(0)
                            : null;
                    moduleStart = 0;
                }
        ...
        if (this.isCategoryHovered(startX + 60, startY, startX + 200, startY + 320, mouseX, mouseY)) {
            if (m < 0 && moduleStart < Client.instance.getModuleManager().getModulesInType(currentModuleType).size() - 1) {
                moduleStart++;
            }
        ...
        FontManager.F16.drawStringWithShadow(
        ...
        if (currentModule != null) {
            float mY = startY + 30;
            for (int i = 0; i < Client.instance.getModuleManager().getModulesInType(currentModuleType).size(); i++) {
...
//                        new Color(40, 40, 40, (int) opacity.getOpacity()).getRGB());
                FontManager.F16.drawStringWithShadow(module.getName(), startX + 90, mY + 9,
                        new Color(248, 248, 248, (int) opacity.getOpacity()).getRGB());
                if (!module.isEnabled()) {
                    RenderUtil.drawFilledCircle(startX + 75, mY + 13, 2,
                            new Color(23, 23, 23, (int) opacity.getOpacity()).getRGB(), 5);
                } else {
            ...
            for (Value value : currentModule.getValues()) {
                ...
                if (value instanceof Mode) {
                    float x = startX + 300;
                    FontManager.F16.drawStringWithShadow(value.getDisplayName(), startX + 210, mY, -1);
                    ...
                    R2DUtils.drawBorderedRect(x - 5, mY - 5, x + 90, mY + 15,
                            new Color(72, 72, 72, (int) opacity.getOpacity()).getRGB(), 2);
                    FontManager.F16.drawStringWithShadow(((Mode) value).getModeAsString(),
                            (float) (x + 44 - FontManager.F16.getStringWidth(((Mode) value).getModeAsString()) / 2), mY + 1, -1);
            ...
            for (Value value : currentModule.getValues()) {
                ...
                if (value instanceof Numbers) {
                    ...
                    FontManager.F16.drawStringWithShadow(value.getDisplayName() + ": " + value.getValue(), startX + 210, mY, -1);
                    if (!Mouse.isButtonDown((int) 0)) {
                        this.previousmouse = false;
                    }
