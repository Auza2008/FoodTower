...
public class Physic {
    ...
    public static void doRenderItemPhysic(Entity par1Entity, double x, double y, double z, float par8, float par9) {
        ...
        if ((itemstack = (item = (EntityItem) par1Entity).getEntityItem()).getItem() != null) {
            ...
            if (TextureMap.locationBlocksTexture != null) {
                Physic.mc.getRenderManager().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
                Physic.mc.getRenderManager().renderEngine.getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false, false);
                flag = true;
            }
            ...
            int i = Physic.func_177077_a(item, x, y - 0.1f, z, par9, ibakedmodel);
            ...
            if (!(item == null || Double.isNaN((double) item.getAge()) || Double.isNaN((double) item.getAir()) || Double.isNaN((double) item.getEntityId()) || item.getPosition() == null)) {
                if (item.onGround) {
                    if (item.rotationPitch != 0.0f && item.rotationPitch != 90.0f && item.rotationPitch != 180.0f && item.rotationPitch != 270.0f) {
                        double Abstand0 = Physic.formPositiv(item.rotationPitch);
                        double Abstand90 = Physic.formPositiv(item.rotationPitch - 90.0f);
                        double Abstand180 = Physic.formPositiv(item.rotationPitch - 180.0f);
                        double Abstand270 = Physic.formPositiv(item.rotationPitch - 270.0f);
            ...
            Physic.mc.getRenderManager().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
            if (flag) {
                Physic.mc.getRenderManager().renderEngine.getTexture(TextureMap.locationBlocksTexture).restoreLastBlurMipmap();
            }
        }
    }
    ...
    public static int func_177077_a(EntityItem item, double x, double y, double z, float p_177077_8_, IBakedModel p_177077_9_) {
        ...
        int i = Physic.getModelCount(itemstack);
        float f1 = 0.25f;
        float f2 = 0.0f;
        GlStateManager.translate((float) ((float) x), (float) ((float) y + f2 + 0.25f), (float) ((float) z));
        float f3 = 0.0f;
        if (flag || Physic.mc.getRenderManager().renderEngine != null && Physic.mc.gameSettings.fancyGraphics) {
